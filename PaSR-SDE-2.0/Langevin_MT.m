...
%       f_kf(npd,nsv)   :   particle-independent scalars
% -----------------------------------------------------------------
function f_kf = LangevinMT(dt,ncell,omean_cell,...
                           np,npd,nindv,nf,nl,f_kwt,f_kf,...
                           min_mixf,max_mixf,istep,dW,d_0,VarMax,var_ini)
    ...
    for lc = 1:1:ncell
        ...
        for i = nf(lc):1:nl(lc)                 
            ...
            % LANGEVIN MODEL VIA IMPLICIT MILSTEIN-TAYLOR METHOD
            ...
            while ( erro > 1e-8 )
                ...
                % compute F(X) MILSTEIN TAYLOR IMPL.
                FX = X + a_cte*omean*( X - fmean )*dt - dW(i,istep)*(2*b_cte*omean*X*(1-X))^half + (1/2)*b_cte*omean*(1-2*X)*(dW(i,istep)^2+dt) - f_kf(i,1);
                
                % compute D_F(X) MILSTEIN TAYLOR IMPL.
                D_FX = 1 + a_cte*omean*dt - dW(i,istep)*(2*b_cte*omean)^half*(1-2*X)/(2*(X*(1-X))^half) - b_cte*omean*(dW(i,istep)^2+dt);
