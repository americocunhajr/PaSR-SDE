...
%       f_kf(npd,nsv)   :   particle-independent scalars
% -----------------------------------------------------------------
function f_kf = ExtLangevin_EI(dt,ncell,omean_cell,...
    ...
    for lc = 1:1:ncell
        ...
        for i = nf(lc):1:nl(lc)                 
            ...
            % EXTENDED LANGEVIN MODEL VIA IMPLICIT EULER METHOD
            ...
            while ( erro > 1e-8 )
                ...
                % compute F(X)
                FX = X + a_cte*OMEGAO*omega_vel(i)*( X - mean(f_kf(:,1).*omega_vel)/mean(omega_vel) )*dt - dW(i,istep)*(2*b_cte*OMEGAO*omega_vel(i)*X*(1-X))^half - f_kf(i,1);
                
                % compute D_F(X)
                D_FX = 1 + a_cte*OMEGAO*omega_vel(i)*dt - dW(i,istep)*(2*b_cte*OMEGAO*omega_vel(i))^half*(1-2*X)/sqrt(2*X*(1-X));
